...
const {
  ...
  const nextLevelAt = level === "Diamond Member" ? points : level === "Gold Member" ? 1500 : 500;
  const progress = Math.min(points / nextLevelAt * 100, 100);
  return <div className="space-y-8">
      <div>
        <h1 className="font-serif italic text-3xl text-ink">Coupons Rewar</h1>
        <p className="text-ink/60 mt-1">View Your Loyal</p>
      </div>
      ...
      <div className="bg-surface/50 backdrop-blur-md border border-line/60 rounded-3xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 relative z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h2 className="text-2xl font-serif italic text-ink">{level}</h2>
            </div>
          </div>
          <div className="md:text-right">
            <p className="text-ink/60 text-sm font-medium uppercase tracking-wider mb-1">Available Point</p>
            <h2 className="text-3xl font-bold text-amber-400">{points.toLocaleString()}</h2>
          </div>
        </div>
        ...
        <div className="relative z-10">
          ...
          <p className="text-ink/50 text-xs mt-3 text-center">"Earn"{nextLevelAt - points}"More Points To"</p>
        </div>
      </div>

      {/* Coupons */}
      <div>
        <h2 className="text-xl font-serif italic text-ink mb-6">Available Coupo</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {coupons.map((coupon, idx) => <motion.div key={coupon.id} initial={{
          opacity: 0,
          scale: 0.95
        }} animate={{
          opacity: 1,
          ...
          delay: idx * 0.1
              ...
              <div className="flex items-center gap-2">
                ...
                {coupon.status === 'Available' && <button onClick={() => handleCopy(coupon.id)} className="p-3 bg-secondary/50 hover:bg-secondary text-ink rounded-xl transition-colors">
