  ...
  return <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-ink/60 mt-1">Manage Your Sav</p>
        </div>
        {!isAdding && <button onClick={() => setIsAdding(true)} className="flex items-center justify-center gap-2 px-6 py-3 bg-primary hover:bg-primary/90 text-white rounded-xl font-medium transition-all shadow-[0_4px_20px_-4px_rgba(233,30,99,0.3)]">
            <FiPlus size={18} />Add Payment Met</button>}
      </div>
          ...
          setIsAdding(false);
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <div className="relative">
                  ...
                  <input required placeholder="0000 0000 0000" className="w-full bg-secondary/30 border border-line/50 rounded-xl pl-11 pr-4 py-3 text-ink focus:outline-none focus:border-primary/50" />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-ink/70 text-sm font-medium">Expiry Date</label>
                <input required placeholder="Mmyy" className="w-full bg-secondary/30 border border-line/50 rounded-xl px-4 py-3 text-ink focus:outline-none focus:border-primary/50" />
              </div>
              <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        return <div key={card.id} className={`bg-gradient-to-br from-surface to-secondary backdrop-blur-md border ${card.isDefault ? 'border-primary/50 shadow-[0_0_20px_-5px_rgba(233,30,99,0.15)]' : 'border-line/60'} rounded-2xl p-6 relative group overflow-hidden`}>
            {/* Glossy overlay effect */}
            <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent pointer-events-none" />
            
            {card.isDefault && <div className="absolute top-4 right-4 flex items-center gap-1.5 text-primary bg-primary/10 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
            ...
            <div className="text-ink tracking-[0.2em] font-mono mb-6 text-lg">{"auto__94"}{card.last4}
            </div>
            ...
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
              <button className="flex items-center justify-center gap-2 px-4 py-2 bg-secondary/80 hover:bg-secondary text-ink rounded-xl text-sm font-medium transition-colors">
                <FiStar size={16} />Set Default</button>
        ...
        {!isAdding && <button onClick={() => setIsAdding(true)} className="flex flex-col items-center justify-center h-full min-h-[220px] bg-secondary/20 hover:bg-secondary/40 border border-dashed border-line/60 rounded-2xl transition-colors group cursor-pointer">
