...
export default function SecuritySettings() {
  ...
  return <div className="space-y-8">
      <div>
        <h1 className="font-serif italic text-3xl text-ink">Security Settin</h1>
        <p className="text-ink/60 mt-1">Keep Your Accou</p>
      </div>
        ...
        <div className="bg-surface/50 backdrop-blur-md border border-line/60 rounded-3xl p-6 md:p-8">
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            <div className="space-y-2">
              <label className="text-ink/70 text-sm font-medium">Current Passwor</label>
            ...
            <div className="space-y-2">
              <label className="text-ink/70 text-sm font-medium">Confirm New Pas</label>
              <input type="password" required className="w-full bg-secondary/30 border border-line/50 rounded-xl px-4 py-3 text-ink focus:outline-none focus:border-primary/50" />
            </div>
            <div className="pt-4">
              <button type="submit" className="w-full py-3 bg-primary hover:bg-primary/90 text-white font-medium rounded-xl transition-all shadow-[0_4px_20px_-4px_rgba(233,30,99,0.3)]">Update Password</button>
            </div>
        ...
        <div className="space-y-8">
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h2 className="text-xl font-serif italic text-ink">Twofactor Auth</h2>
                </div>
                <p className="text-ink/50 text-sm pl-13">Add An Extra La</p>
              </div>
          ...
          <div className="bg-surface/50 backdrop-blur-md border border-line/60 rounded-3xl p-6 md:p-8">
            ...
            <div className="space-y-4">
              ...
              return <div key={device.id} className="flex items-start justify-between p-4 bg-secondary/30 rounded-xl border border-line/30">
                  ...
                  {!device.active && <button className="text-red-400 hover:text-red-300 text-sm font-medium transition-colors p-2">
                      <FiLogOut size={16} />
                    </button>}
                </div>;
            })}
            </div>
            <button className="w-full mt-6 py-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 font-medium rounded-xl transition-all">Log Out Of All</button>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        ...
        scale: 0.95
            ...
            <FiCheck size={20} />Password Update</motion.div>}
      </AnimatePresence>
    </div>;
}