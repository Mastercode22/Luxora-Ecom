...
export default function ShopProductCard({ product, viewMode, onQuickView }) {
  ...
  if (viewMode === 'list') {
    return (
      <Link to={`/product/${product.slug}`} className="group flex flex-col sm:flex-row gap-6 p-4 bg-surface/40 hover:bg-surface/80 backdrop-blur-md border border-line/60 rounded-3xl transition-all duration-300 hover:shadow-2xl hover:border-primary/30">
        <div className="relative w-full sm:w-64 h-48 rounded-2xl overflow-hidden shrink-0 bg-secondary/30">
          ...
          {product.badge && (
            <div className="absolute top-3 left-3 px-3 py-1 bg-ink/90 backdrop-blur-md text-surface text-xs font-bold tracking-wider uppercase rounded-full">
              {product.badge}
            </div>
          )}
        </div>
        <div className="flex-1 flex flex-col justify-center">
          ...
          <div className="flex items-center justify-between mt-auto">
            ...
            <div className="flex gap-2">
              <button onClick={handleQuickView} className="px-5 py-2.5 rounded-xl bg-secondary/50 text-ink font-medium text-sm hover:bg-secondary transition-colors">
                Quick View
              </button>
              <button onClick={handleAddToCart} className="px-6 py-2.5 rounded-xl bg-ink text-surface font-medium text-sm hover:bg-primary transition-colors">
                Add to Cart
              </button>
            </div>
          </div>
  ...
  return (
    <Link to={`/product/${product.slug}`} className="group block relative bg-surface border border-line/60 rounded-3xl overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-1">
      {/* Image Container */}
      <div className={`relative w-full overflow-hidden bg-secondary/20 ${viewMode === 'compact' ? 'h-48' : 'h-72 lg:h-80'}`}>
        ...
        <div className="absolute top-4 left-4 right-4 flex justify-between items-start z-10 pointer-events-none">
          {product.badge ? (
            <span className="px-3 py-1.5 bg-ink/90 backdrop-blur-md text-surface text-xs font-bold tracking-wider uppercase rounded-full shadow-lg">
              {product.badge}
            </span>
          ) : <div />}
      ...
      <div className={`p-5 ${viewMode === 'compact' ? 'p-4' : 'p-6'}`}>
        ...
        {viewMode !== 'compact' && (
          <p className="text-ink/50 text-sm line-clamp-2 mb-4 h-10">
            {product.description}
          </p>
        )}
        <div className="flex items-end gap-2 mt-2">
          ...
          {product.oldPrice && (
            <span className="text-sm text-ink/40 line-through mb-[2px]">${product.oldPrice}</span>
          )}
        </div>
      </div>
    </Link>
