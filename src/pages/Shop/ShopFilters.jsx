...
export default function ShopFilters({ filters, setFilters, products, isMobileDrawer, isOpen, onClose }) {
  ...
  const clearAll = () => {
    setFilters({ categories: [], brands: [], priceRange: [0, 5000], badges: [] });
  };
      ...
      {activeFilterCount > 0 && (
        <div className="flex flex-wrap gap-2">
          {filters.categories.map(c => (
             <div key={c} className="flex items-center gap-1.5 px-3 py-1 bg-primary/10 text-primary border border-primary/20 rounded-full text-xs font-medium">
               {c} <button onClick={() => handleCategoryToggle(c)}><FiX size={12} /></button>
             </div>
          ))}
          {filters.brands.map(b => (
             <div key={b} className="flex items-center gap-1.5 px-3 py-1 bg-primary/10 text-primary border border-primary/20 rounded-full text-xs font-medium">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
          {categories.map(([cat, count]) => (
            <button key={cat} type="button" onClick={() => handleCategoryToggle(cat)} className="w-full flex items-center justify-between cursor-pointer group">
      ...
      <div className="space-y-4 pt-6 border-t border-line/30">
        ...
        <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
          {brands.map(([brand, count]) => (
            <button key={brand} type="button" onClick={() => handleBrandToggle(brand)} className="w-full flex items-center justify-between cursor-pointer group">
              <div className="flex items-center gap-3">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex-1 bg-surface border border-line/60 rounded-lg px-3 py-2 flex items-center gap-2">
              ...
              <input 
                type="number" 
                value={filters.priceRange[1]} 
                onChange={e => setFilters(prev => ({...prev, priceRange: [prev.priceRange[0], parseInt(e.target.value) || 5000]}))}
                className="w-full bg-transparent text-sm text-ink focus:outline-none"
              />
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {badges.map(badge => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${filters.badges.includes(badge) ? 'bg-primary text-white shadow-[0_0_15px_-3px_rgba(233,30,99,0.4)]' : 'bg-surface border border-line/60 text-ink/70 hover:border-primary/50 hover:text-ink'}`}
            >
              {badge}
            </button>
          ))}
        ...
        {isOpen && (
          <>
            <motion.div 
              ...
              onClick={onClose}
              className="fixed inset-0 bg-ink/70 backdrop-blur-sm z-[9998]"
            />
            <motion.div 
            ...
            >
              ...
              <div className="sticky bottom-0 p-6 bg-surface/90 backdrop-blur-md border-t border-line/30">
                <button onClick={onClose} className="w-full py-4 bg-primary text-white rounded-xl font-bold shadow-lg shadow-primary/25">
                  Apply Filters
                </button>
              </div>
            </motion.div>
  ...
  return (
    <div className="sticky top-[130px] p-6 bg-surface/50 backdrop-blur-md border border-line/60 rounded-3xl">
      <FilterContent />
    </div>
  );
}
