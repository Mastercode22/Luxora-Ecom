...
import { FiDownload, FiEye, FiSearch, FiPackage, FiX, FiPrinter } from "react-icons/fi";
...
export default function OrderHistory() {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      default: return "bg-secondary text-ink/70";
    }
  };
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-ink/60 mt-1">Track, return, or purchase items again.</p>
        </div>
        ...
        <div className="relative">
          ...
          <input 
            ...
            className="w-full sm:w-64 bg-secondary/30 border border-line/60 rounded-xl pl-11 pr-4 py-3 text-ink focus:outline-none focus:border-primary/50 text-sm transition-colors"
          />
        </div>
      </div>
      ...
      <div className="bg-surface/50 backdrop-blur-md border border-line/60 rounded-3xl overflow-hidden">
            ...
            <tbody className="divide-y divide-line/30">
              {user.orders.map((order, idx) => (
                ...
                >
                  ...
                  <td className="p-6 text-ink/70">{order.date}</td>
                  <td className="p-6 text-ink/70">{order.items} items</td>
                  <td className="p-6 text-ink font-medium">${order.total.toFixed(2)}</td>
                  <td className="p-6">
                  ...
                  <td className="p-6">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={() => { setSelectedOrder(order); setTimeout(handlePrint, 300); }} className="p-2 bg-secondary/50 hover:bg-secondary text-ink rounded-lg transition-colors tooltip-trigger" title="Download Invoice">
                        <FiDownload size={16} />
                      </button>
                    </div>
                  </td>
          ...
          {user.orders.map((order, idx) => (
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-ink font-bold text-lg mb-1">{order.id}</h3>
                  <p className="text-ink/50 text-sm">{order.date}</p>
                </div>
                <span className={`px-3 py-1.5 rounded-full text-xs font-bold border ${getStatusColor(order.status)}`}>
                  {order.status}
                </span>
              ...
              <div className="flex items-center justify-between py-4 border-y border-line/30">
                ...
                <div className="text-ink font-bold">${order.total.toFixed(2)}</div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => { setSelectedOrder(order); setTimeout(handlePrint, 300); }} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-secondary/50 hover:bg-secondary text-ink rounded-xl text-sm font-medium transition-colors">
        ...
        {user.orders.length === 0 && (
          <div className="p-12 text-center">
            ...
            <p className="text-ink/50 max-w-md mx-auto">When you place an order, it will appear here so you can easily track its status.</p>
          </div>
        )}
      </div>
        ...
        {selectedOrder && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            <motion.div
              ...
              className="absolute inset-0 bg-ink/60 backdrop-blur-sm print:hidden"
            />
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-surface border border-line/60 rounded-3xl w-full max-w-2xl relative z-10 overflow-hidden shadow-2xl flex flex-col max-h-[90vh]"
              id="invoice-modal"
            >
              {/* Header */}
              <div className="p-6 border-b border-line/30 flex items-center justify-between bg-secondary/20">
                <div>
                  <h2 className="text-2xl font-serif italic text-ink">Order #{selectedOrder.id}</h2>
                  <p className="text-ink/60 text-sm mt-1">{selectedOrder.date}</p>
                </div>
              ...
              <div className="p-6 overflow-y-auto print:overflow-visible">
                <div className="flex justify-between items-start mb-8">
                  <div>
                    <h3 className="text-sm font-bold text-ink/50 uppercase tracking-wider mb-2">Shipping Details</h3>
                    <p className="text-ink font-medium">{user.profile.fullName}</p>
                    <p className="text-ink/70 text-sm">{user.profile.email}</p>
                    <p className="text-ink/70 text-sm mt-1">123 Luxury Avenue, Apt 4B<br />New York, NY 10001</p>
                  </div>
                  <div className="text-right">
                    <h3 className="text-sm font-bold text-ink/50 uppercase tracking-wider mb-2">Payment Info</h3>
                    <p className="text-ink font-medium">{selectedOrder.paymentMethod}</p>
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-4">
                    {Array.from({ length: selectedOrder.items }).map((_, i) => (
                      <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-secondary/30 border border-line/30">
                        ...
                        <p className="text-ink font-medium">${(selectedOrder.total / selectedOrder.items).toFixed(2)}</p>
                      </div>
                    ))}
                  </div>
                </div>
