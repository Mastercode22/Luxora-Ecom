...
export default function SecuritySettings() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-serif italic text-3xl text-white">Security Settings</h1>
        <p className="text-white/60 mt-1">Keep your account secure and manage devices.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-surface/50 backdrop-blur-md border border-line/60 rounded-3xl p-6 md:p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-serif italic text-white">Change Password</h2>
          </div>
          
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            <div className="space-y-2">
              <label className="text-white/70 text-sm font-medium">Current Password</label>
              <input type="password" required className="w-full bg-secondary/30 border border-line/50 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-primary/50" />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <label className="text-white/70 text-sm font-medium">Confirm New Password</label>
              <input type="password" required className="w-full bg-secondary/30 border border-line/50 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-primary/50" />
            </div>
            <div className="pt-4">
            ...
            <p className="text-white/40 text-xs text-center mt-4">Last changed: {user.security.lastPasswordChange}</p>
          </form>
        </div>
        ...
        <div className="space-y-8">
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h2 className="text-xl font-serif italic text-white">Two-Factor Auth</h2>
                </div>
                <p className="text-white/50 text-sm pl-13">Add an extra layer of security to your account.</p>
              </div>
          ...
          {/* Active Devices */}
          <div className="bg-surface/50 backdrop-blur-md border border-line/60 rounded-3xl p-6 md:p-8">
            <h2 className="text-xl font-serif italic text-white mb-6">Active Devices</h2>
              ...
              {user.security.devices.map(device => (
                <div key={device.id} className="flex items-start justify-between p-4 bg-secondary/30 rounded-xl border border-line/30">
                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-full bg-secondary/50 flex items-center justify-center text-white shrink-0">
                    ...
                    <div>
                      ...
                      <p className="text-white/50 text-xs mt-1">{device.location} • {device.time}</p>
                    </div>
                  </div>
                  {!device.active && (
