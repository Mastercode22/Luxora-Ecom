...
const {
  ...
  return <AnimatePresence mode="wait">
      <motion.section key={theme} initial={{
      opacity: 0,
      scale: 0.98,
      filter: "blur(10px)"
    }} animate={{
      ...
      ease: [0.22, 1, 0.36, 1]
        ...
        <Container className="relative z-10 flex flex-col items-center" style={{
        gap: "clamp(40px, 5vw, 60px)"
          ...
          <div className="text-center">
            ...
            ease
              ...
              <div className="inline-flex items-center justify-center gap-2 px-4 py-1.5 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-bold uppercase tracking-widest mb-6">
                <FiHeart size={14} />Kind Words</div>
          ...
          <div className="w-full max-w-5xl mx-auto px-4 md:px-0">
            <Swiper modules={[Autoplay, Pagination, EffectCoverflow]} effect={"coverflow"} grabCursor={true} centeredSlides={true} slidesPerView={"auto"} coverflowEffect={{
            rotate: 0,
            stretch: 0,
            depth: 100,
            modifier: 2,
            ...
            clickable: true,
            el: ".testimonial-custom-dots"
          }} loop className="testimonial-swiper py-12">
              {testimonials.map(t => <SwiperSlide key={t.id} style={{
              width: "100%",
              maxWidth: 500
            }}>
                  <motion.div className="bg-surface/40 backdrop-blur-2xl border border-line/60 rounded-[32px] p-8 md:p-10 flex flex-col items-center text-center relative overflow-hidden group" style={{
                gap: 24,
                boxShadow: theme === 'dark' ? "0 20px 40px -10px rgba(0,0,0,0.3)" : "0 20px 40px -10px rgba(0,0,0,0.05)"
              }}>
                    ...
                    <div className="flex items-center gap-1.5">
                      {Array.from({
                    length: 5
                  }).map((_, i) => <FiStar key={i} size={15} className="fill-current text-primary" />)}
                    </div>
