...
export default function GlobalBackground() {
  ...
  return (
    ...
    >
      ...
      />
      <motion.div
        className="absolute top-[20%] -right-[10%] w-[60vw] h-[60vw] rounded-full mix-blend-screen"
        animate={{ 
          x: [0, -120, 0], 
          y: [0, 80, 0],
          scale: [1, 1.15, 1]
      ...
      />
      <motion.div
        className="absolute -bottom-[20%] left-[20%] w-[40vw] h-[40vw] rounded-full mix-blend-screen"
        animate={{ 
          x: [0, 80, 0], 
          y: [0, -100, 0],
          scale: [1, 1.2, 1]
      ...
      >
        <img 
          src={`${import.meta.env.BASE_URL}sneaker.png`} 
          alt="Sneaker Watermark" 
          className="w-[85vw] max-w-[900px] object-contain"
        />
      </motion.div>
      ...
      {Array.from({ length: 12 }).map((_, i) => {
        ...
        return (
          <motion.div
            key={i}
            className={`absolute ${isCircle ? 'rounded-full' : 'rounded-[2rem]'} border`}
            style={{
              top: `${top}%`,
              left: `${left}%`,
              width: `${size}vw`,
